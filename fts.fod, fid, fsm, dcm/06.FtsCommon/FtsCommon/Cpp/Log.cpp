...
// -------------------------------------------------------------------
bool CLog::SetLogDirectory(const char* LogDirectory)
{
	// Log Folder 설정
...
// -------------------------------------------------------------------
bool CLog::SetLogName(const char* LogName)
{
	m_LogName = LogName;
	return true;
}
...
// Return 값	: true		Always
// -------------------------------------------------------------------
bool CLog::SetFileSaveUnit(EnumLogFileSaveUnit SaveUnit)
{
...
//				  false		FAIL
// -------------------------------------------------------------------
FILE* CLog::Open(  CTime& p_ctCurTime  )
{
	CString strFileName;
	...
	if( m_WriteFd )
	{
		...
		case LOG_FILE_SAVE_UNIT_HOUR:
			if( p_ctCurTime.GetHour() == m_FileOpenTime.GetHour() )
				return m_WriteFd;
		...
		default:
			if( p_ctCurTime.GetHour() == m_FileOpenTime.GetDay() )
				return m_WriteFd;
	...
	case LOG_FILE_SAVE_UNIT_HOUR:	// 시간별 저장
		strFileName.Format( "%s%s_%s.log", m_LogDir.c_str(), m_LogName.c_str(), (LPCSTR)p_ctCurTime.Format("%Y%m%d_%H") );
		break;

	case LOG_FILE_SAVE_UNIT_DAY:	// 일자별 저장
	default:				
		strFileName.Format( "%s%s_%s.log", m_LogDir.c_str(), m_LogName.c_str(), (LPCSTR)p_ctCurTime.Format("%Y%m%d") );
		break;
	}

	m_WriteFd = fopen( strFileName, "a+" );
	if( m_WriteFd == NULL )
		return NULL;


	// 파일 연 일시 업데이트
	m_FileOpenTime = p_ctCurTime;
...
// Return 값	: true		Always
// -------------------------------------------------------------------
bool CLog::Print(int DebugLevel, char* Format, ...)
{	
...
// Return 값	: true		Always
// -------------------------------------------------------------------
bool CLog::Print(int DebugLevel, unsigned int Key, char* Format, ... )
{
	...
	if(m_WriteFd == NULL)
	{
		...
		case LOG_FILE_SAVE_UNIT_HOUR:
			sprintf_s( FileName, "%s%s_%04d%02d%02d_%02d.log", 
						m_LogDir.c_str(), m_LogName.c_str(),
...
void CLog::LogSip( char p_chType, int p_chan, int p_msgid, int p_nLen, int p_result )
{
...
void CLog::LogSip( char p_chType, int p_chan, int p_msgid, int p_nLen, const char* p_szMsg )
{
	int		logLevel;
	CString	strChannel;
...
void CLog::LogSip( int p_logLevel, const char* p_szMsg )
{
...
// Return 값	: true		Always
// -------------------------------------------------------------------
void CLog::PrintHexaData(void* pData, int DataSize, string &PrintData )
{
...
bool CLog::_print( int DebugLevel, const char* p_szLog )
{
	FILE* Fw;
	CTime ctCurTime = CTime::GetCurrentTime();
	
	{CSingleLock Locker( &m_Sync, TRUE );
		
		Fw = Open( ctCurTime );
		if( !Fw )
			return false;
	
		fprintf_s( Fw, "[%s][%s] : %s\n", m_DBGLVStr[DebugLevel], ctCurTime.Format("%Y/%m/%d %H:%M:%S"), p_szLog );			
		fflush( Fw );
...
bool CLog::_print( CString* p_pstrLogLine, int DebugLevel, const char* p_szLog )
{
	FILE* Fw;
	CTime ctCurTime = CTime::GetCurrentTime();
	...
	{CSingleLock Locker( &m_Sync, TRUE );
		...
		p_pstrLogLine->Format( "[%s][%s] : %s\n", m_DBGLVStr[DebugLevel], ctCurTime.Format("%Y/%m/%d %H:%M:%S"), p_szLog );			
		fprintf_s( Fw, (LPCSTR)(*p_pstrLogLine) );
		fflush( Fw );
