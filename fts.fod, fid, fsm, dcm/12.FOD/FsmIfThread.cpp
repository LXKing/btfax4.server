...
bool CFsmIfThread::IsRegistered()
{
	return ( m_sip_status == SIP_Register );
}
...
int CFsmIfThread::FASIPRegister()
{
	...
	pRegister->fax_direction	= 'O';
	strcpy_s( pRegister->fax_ip, CConfig::SYSTEM_IP );
	...
	pRegister->fax_device_cnt	= CConfig::FAX_TOTAL_CH_CNT;

	// P4. Register Packet Send
...
int CFsmIfThread::FASIPKeepalive()
{
	...
	len = Send( (char*) &faxKeepAlive, sizeof(FAX_SIP_HEADER) );
	if( len < 0 )
...
int CFsmIfThread::ReadPacket(SOCKET hInSocket, char *pszBuf4Recv, int nSize, int nTimeOut)
{
    ...
    //memcpy(szPacket, pszBuf4Recv, nRC);

    //for(int i = 0; i< nRC; i++)
    //{
    //    if(szPacket[i] == '\0')
    //    {
    //        APPLOG->Print(DBGLV_ERR, "Find NULL char POS:%d", i);
    //        szPacket[i] == ' ';
    //    }
    //}

    //APPLOG->Print(DBGLV_ERR, "[FOD <-- FSM][%d] %s", nRC, szPacket);
    //// TEST - END
